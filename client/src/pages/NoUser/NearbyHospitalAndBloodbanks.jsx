...
console.log(data);
          ...
          city: props.city.toLowerCase() || props.country.toLowerCase() || props.suburb.toLowerCase() || "",
            });
  ...
  >
    <img
      src={`https://api.geoapify.com/v1/icon/?type=material&color=%23d32f2f&icon=${sug.properties.result_type || "location"}&apiKey=${GEOAPIFY_API_KEY}`}
      alt="icon"
      className="w-5 h-5"
    />
    <span>{sug.properties.formatted}</span>
      ...
      {/* Real-time Nearby Hospitals */}
      <div className="mb-10">
        <h2 className="text-4xl font-semibold text-slate-800 shadow-red-600  mb-3">Nearby Hospitals</h2>
        {nearbyHospitals.length ? (
          <ul className="space-y-4">
            {nearbyHospitals.map((place, idx) => (
              <li key={idx} className="border p-4 rounded shadow bg-white">
      ...
      >
         <p className="font-semibold text-sm mb-1">🗺️ <span className="text-blue-600">View on Map:</span></p>
        {/* <img
      ...
      <p> Distance : {(place.properties.distance / 1000).toFixed(2)} km away
</p>
    </div>
               <p>Type: {place.properties.categories?.join(", ")}</p>
{place.properties.contact?.phone && (
  <p className="text-lg">📞 Phone: {place.properties.contact.phone}</p>
)}
{place.properties.website && (
  <p>
    🌐 Website:{" "}
    <a href={place.properties.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
      {place.properties.website}
    </a>
  </p>
)}
{/* <p>⭐ Rating: <span className="text-yellow-500">Not available</span></p> */}
      ...
      {/* Real-time Nearby Blood Banks */}
      <div className="mb-10">
        <h2  className="text-4xl font-semibold text-slate-800 shadow-red-600  mb-3">Nearby Blood Banks</h2>
        {nearbyBloodBanks.length ? (
          <ul className="space-y-4">
            {nearbyBloodBanks.map((place, idx) => (
              <li key={idx} className="border p-4 rounded shadow bg-white">
